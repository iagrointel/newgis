...
## Rede de utilidades — área suja e validação incremental (item L4-03-d-areas-sujas-e-validacao, turno 4; ADR 20260907T1243)

Continuação de L4-03-a: aqui a rede passa a marcar o que mudou e a validar só o que mudou, como a *utility
network*/*trace network* fazem. Fontes: `dirty-areas-in-a-utility-network.htm`,
`validate-network-topology.htm`, `about-error-features.htm`, `manage-error-features.htm`,
`dirty-areas-in-a-trace-network.htm` (ArcGIS Pro 3.4). Não conferido por HTTP contra Pro/AGOL reais
(pendente D20), leitura do modelo documentado.

| capacidade | Esri | nós | estado | testado por | data | Pro/AGOL real |
|---|---|---|---|---|---|---|
| área suja (*dirty area*) | polígono marcado por feição editada, por versão de edição | `plat.rede_area_suja`: um polígono envolvente (buffer 2 m) por feição tocada em `applyEdits` (adicionada, atualizada ou apagada — pela geometria capturada antes do DELETE), carimbado com `plat.rede.versao_edicao` | feito | `test_editar_uma_feicao_cria_uma_area_suja_visivel_no_mapa`, `test_feicao_apagada_tambem_cria_area_suja_pela_geometria_anterior`, `test_500_edicoes_em_lote_criam_no_maximo_500_areas_sujas` | 2026-09-07 | pendente (D20) |
| validar extensão / validar tudo (*Validate Network Topology*) | reconstrói a topologia só nas áreas sujas tocadas pela extensão pedida (ou todas) | `POST /api/rede/{id}/validar_extensao`: escopo = união das áreas sujas ativas que tocam `extensao` (ou todas, `extensao: null`); só as feições do escopo entram nas 15 checagens; áreas processadas viram limpas | feito | `test_validar_extensao_pequena_limpa_so_a_area_e_reconstroi_menos_que_o_total`, `test_validar_extensao_pequena_nao_altera_o_checksum_do_resto_da_rede` | 2026-09-07 | pendente (D20) |
| feição de erro (*error feature*) | erro de validação vira feição com código, descrição e geometria, consultável como camada | `plat.rede_erro` (`codigo`, `mensagem`, `feicao_id`, `tipo_referencia`, `detalhe`, `geometria`); `GET /api/rede/{id}/erros` devolve `FeatureCollection`; revalidar um escopo pequeno substitui só os erros das feições daquele escopo | feito | `tests/api/test_areas_sujas_e_validacao.py` (as 14 checagens provocadas) | 2026-09-07 | pendente (D20) |
| 15 códigos de validação (regras de conectividade + estruturais) | famílias de erro documentadas por tipo de regra e checagem estrutural | 2 em tempo de escrita (`sem_regra`, `terminal_errado`, item L4-03-a) + 13 na validação (`regra_inexistente`, `terminal_invalido`, `terminal_obrigatorio_ausente`, `feicao_sem_conexao`, `sobreposicao_dispositivo`, `ciclo_tier_hierarquico`, `subrede_sem_controlador`, `atributo_obrigatorio_nulo`, `geometria_invalida`, `associacao_ciclo`, `feicao_duplicada_geometria`, `tipo_sem_regra_no_pacote`, `atributo_tipo_invalido`) | feito, com ressalva | 14 de 15 com teste de ponta a ponta; `regra_inexistente` é defensivo, inalcançável pela API com a FK corrigida (migração 20260907T1308) | 2026-09-07 | pendente (D20) |
| traçado sobre área suja (*dirty areas in a trace network*) | traçado que começa numa área suja é avisado ou recusado, conforme configuração | `GET /api/rede/{id}/tracar` (feição ou geometria solta); `PUT /api/rede/{id}/area_sujas/modo` troca `avisar`/`bloquear` (`rede.administrar`); os dois devolvem o polígono da área | feito | `test_tracado_sobre_area_suja_avisa_por_padrao_com_o_poligono`, `test_tracado_sobre_area_suja_bloqueia_no_modo_bloquear` | 2026-09-07 | pendente (D20) |
| *Verify Network Topology* (validação sem gravar erro) / *Repair Network Topology* (reparo automático de geometria) | ferramentas separadas de validação só-leitura e de reparo automático | não existem: `validar_extensao` sempre grava; não há reparo automático de geometria inválida | fora (ver ADR, fronteira honesta) | — | 2026-09-07 | pendente (D20) |
