...
developers.arcgis.com/experience-builder e doc.arcgis.com/en/experience-builder/latest/configure-widgets/add-actions-to-widgets.htm,
acesso 2026-09-05); Dashboards, "Actions" e "Relationships between data sources" (doc.arcgis.com/en/dashboards, mesma data).

| capacidade | Esri | nós | estado | testado por | data | Pro/AGOL real |
|---|---|---|---|---|---|---|
| fonte de dado e vista de dado referenciadas por id | data source / data view (Experience Builder) | `corpo.fontes[]` (item do catálogo, caminho do servidor ou embutida; campos tipados) e `corpo.vistas[]` (fonte + filtro CQL2-JSON + seleção + ordenação + campos), ULID cada; widgets ligam por `configuracao.vista` | feito | testador (unit node+python, api, e2e) | 2026-09-08 | pendente (D20) |
| formato do filtro | SQL `where` (Esri) | CQL2-JSON (OGC 21-065r2), CQL2-text aceito na entrada e convertido; mesmo subconjunto que o L2-04-g compila para SQL | feito (=, <>, <, <=, >, >=, and/or/not, in, like, between, isNull, s_intersects por envelope) | `test_cql2_texto_e_json_filtram_igual` | 2026-09-08 | pendente (D20) |
| gatilhos (Experience Builder "action triggers") | Record selection changes · Data filtering changes · Extent changes · Data records loaded · Data source changed (view) · Record added · Location changes · Button click | `selecao_mudou` · `filtro_mudou` · `extensao_mudou` · `registros_carregados` · `vista_mudou` · `dado_adicionado` · `localizacao` · `clique` (8 de 8) | feito (vocabulário fechado no esquema 3 do documento e no manifesto de cada widget) | esquema `GET /api/esquemas/app`; `test_esquema_publicado_tem_as_tres_listas` | 2026-09-08 | pendente (D20) |
| ações de dado | Filter data records · Select data records · Clear data filter · Clear selection | `filtrar` · `selecionar` · `limpar_filtro` · `limpar_selecao`, resolvidas na VISTA do alvo (quem filtra a vista muda todos os widgets dela) | feito | e2e (seleção no mapa refiltra tabela e gráfico da mesma fonte) | 2026-09-08 | pendente (D20) |
| ações de widget/mapa (Dashboards "Actions": filter, set extent, flash, show pop-up, pan, zoom, follow feature) | 7 | `zoom` · `pan` · `piscar` · `popup` · `abrir` · `fechar` · `definir_parametro` (seguir feição fora: sem rastreio ao vivo neste item) | parcial (6 de 7 + abrir/fechar/parâmetro) | node (`piscar` no gráfico a cada disparo), e2e | 2026-09-08 | pendente (D20) |
| relação entre fontes diferentes | Dashboards: atributo (tipos iguais; inteiro/decimal e data/data-hora equivalem) ou espacial | `relacao: mesma_fonte / atributo / espacial` com a MESMA regra de tipos; sem relação = recusado no construtor (mensagem) e na API (422 `modelo_invalido`, regra `relacao_ausente`); tipos que não casam = `relacao_tipos` | feito | unit (python == node), api, e2e (`#mensagem-erro`) | 2026-09-08 | pendente (D20) |
| estado (seleção e filtros) na URL | parâmetros de URL do Experience Builder só para dado/página; seleção não | `v.<vista>` por vista (base64url de {f, s}); URL copiada reabre igual; estado restaurado ANTES de as mensagens ouvirem (não redispara) | feito (acima da Esri para seleção) | node (ida e volta), e2e (novo contexto de navegador) | 2026-09-08 | pendente (D20) |
