...
| `applyEdits`, anexos, `queryRelatedRecords`, `relationships` | edição transacional, upload de anexo, registros relacionados | **desatualizado desde que o item da edição entrou**: `applyEdits` e as rotas de anexo existem e são medidas na matriz de conformidade deste documento; `queryRelatedRecords` e `relationships` continuam fora | ver a matriz de conformidade (seção gerada) | esta linha ficou aqui como registro do que valia quando o item foi escrito; a seção gerada é a que vale hoje |
