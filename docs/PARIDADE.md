...
| *network category* | rótulo atribuído a tipos de ativo (fonte, dispositivo de proteção, seccionamento) e consultado no traçado | `plat.rede_categoria` + `plat.rede_tipo_categoria`; 12 categorias no pacote elétrico (era 11: acrescida `derivacao`, item L4-06-d), 8 no de água; categoria citada por um tipo e não declarada no pacote é recusada na importação; alterar o conjunto de um tipo marca área suja em toda feição já instanciada dele | feito (declaração + consumo pelo traçado de isolamento; o motor de traçado completo com múltiplas fases é item de linha própria) | `test_pacote_eletrica_br_tem_pelo_menos_8_categorias`, `test_redefinir_categorias_marca_area_suja_e_devolve_contagem`, `test_isolamento_para_na_categoria_protecao` (T4, L4-06-d) | 2026-09-06 | pendente (D20) |
| *feature restrictions* | restrição de feição por tipo de ativo (não pode ser ponto de partida de traçado, não admite terminal) | `plat.rede_tipo_restricao` (`sem_ponto_partida`, `sem_terminal`); o traçado de isolamento recusa (422) partir de feição cujo tipo tem `sem_ponto_partida` | feito (vocabulário fechado às duas restrições citadas no portão do item; a Esri tem um conjunto maior) | `test_restricao_sem_ponto_partida_bloqueia_isolamento`, `test_restricao_desconhecida_e_recusada` (T4, L4-06-d) | 2026-09-06 | pendente (D20) |
| *subnetwork tap* | ponto de derivação de subrede: feição de ponto com um único terminal onde o traçado de subrede para | categoria `derivacao` no pacote `eletrica-br`, atribuída a `unidade_consumidora/1` (ponto, um terminal — mesma condição de elegibilidade da Esri) | parcial (a categoria e a condição de elegibilidade existem; o traçado de SUBREDE propriamente dito — que trata `derivacao` como fim de ramal — não foi construído, só o de isolamento, que para em `dispositivo_de_protecao`) | `test_pacote_eletrica_br_tem_pelo_menos_8_categorias` confere a categoria existe; NENHUM teste exercita o comportamento de "parar no tap" | 2026-09-06 | pendente (D20) |
