...
| regras de conexão, contenção e fixação (*network rules*, add/import/export rule) | *Junction-Junction*, *Junction-Edge*, *Edge-Junction-Edge Connectivity*, *Containment*, *Structural Attachment*; "features can only connect or associate if a rule exists" | `plat.rede_regra` com os cinco tipos (`juncao_juncao`, `juncao_aresta`, `aresta_juncao_aresta`, `contencao`, `estrutura`); 58 regras no pacote `eletrica-br` (≥ 40 exigidos pelo portão); imposta em `POST /api/rede/{id}/applyEdits` (recusa 409 com código e regra candidata citada) e em `POST /api/rede/{id}/validar` (lote); `regras_ativas` é a única comporta, só `rede.administrar`; CSV nas 13 colunas Import/Export Rules do ArcGIS Pro (importação SUBSTITUI, a da Esri ACRESCENTA — diferença documentada no ADR) | feito | `tests/api/test_regras_conectividade.py`, `tests/api/test_regras_csv.py`, `tests/unit/test_regras_motor.py` (T4) | 2026-09-07 | pendente (D20) |
...
| conferência cruzada entre motores | não existe | o MESMO alimentador resolvido no OpenDSS (trifásico) e no pandapower (sequência positiva) dá tensão de barra dentro de ±1 % barra a barra — medido: diferença máxima ~1e-4 pu | feito (medido) | `tests/unit/test_matpower_e_pandapower.py::test_conferencia_cruzada_opendss`; medida em `tests/medidas/L4-05-c-pandapower-e-matpower.json` | 2026-09-18 | não se aplica |
...
| `mosaicRule` | regra de mosaico aplicada no servidor | RECUSADO explicitamente sempre que vem com valor — depende do item L1-07 (mosaico de coleção; construído no mesmo turno por outra trilha, mas a LIGAÇÃO entre `mosaicRule` e `plat.mosaico` não foi feita — ver `docs/PARIDADE.md` linha do L1-07) | fora (dependência existe, ligação não feita) | `test_imageserver_token.py::test_export_image_mosaic_rule_e_recusado` | 2026-09-10 | pendente (D20) |
...
| regra de seleção de pixel (Closest to Center, By Attribute, Lock Raster, Northwest, Seamline, Closest to Viewpoint) | 7 métodos de `mosaicRule` | só `first` (primeira cena com dado, ordem do `sortby` registrado — padrão data desc); os outros 6 são o item irmão L1-08, não construído | fora (L1-08) | — | 2026-09-10 | pendente (D20) |
...
| `mosaicRule` do ImageServer compatível Esri (L1-25) usa o mosaico registrado | `exportImage`/`tile` com `mosaicRule` aplicam o mosaic dataset | AINDA NÃO LIGADO: a rota `/ImageServer` continua recusando `mosaicRule` com valor — a dependência (este item) agora existe, mas ninguém escreveu a ponte entre o parâmetro Esri e `plat.mosaico`; `rotas_imageserver.py` estava sendo tocado por outra trilha (predefinição de renderização) no mesmo turno, e ligar as duas juntas era risco maior que o orçamento deste turno permitia | fora (dependência agora existe; ligação não feita — ver `docs/PARIDADE.md` linha `mosaicRule` na seção L1-25 e ADR §5) | — | 2026-09-10 | pendente (D20) |
