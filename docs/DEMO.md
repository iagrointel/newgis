...
- não prometer: O mapa mostra hoje a base local. Camadas de dados do catálogo sobre o mapa, edição
  geográfica e imagens de satélite ainda não existem no produto.
...
- não prometer: O construtor cobre páginas e layout, e está em construção. Widgets de dado,
  publicação por link e modelos prontos ainda não existem. O construtor é hoje a parte do produto com
  menos paridade com o ArcGIS Online.
...
- Imagens de satélite, COG (Cloud Optimized GeoTIFF), tiles de imagem e URL WMTS (Web Map Tile
  Service) em cliente OGC (Open Geospatial Consortium). A linha L1 não tem nenhum item entregue no
  painel da versão avaliada. Dizer que é a próxima linha do plano. Dizer que hoje nada dela pode ser
  demonstrado.
- Edição no mapa e serviços OGC ou Esri-compatíveis. Itens L2-03 e L2-04 pendentes. O mapa desenha a
  base local. Ele ainda não desenha camadas de dados nem aceita edição.
- Motor multicritério com tela. O modelo de dado do motor existe (L3-01-a entregue). A tela não
  existe (L3-01-g pendente). Dizer que o motor é serviço interno em construção, sem demonstração na
  interface hoje.
- Traçado de rede elétrica. A linha L4 está inteiramente pendente. Dizer que é uma linha própria do
  plano. Dizer que não há nada para mostrar.
- Acervo da casa com tela. O registro de procedência existe (L6-01-a entregue). A tela do acervo
  não existe (L6-01-c pendente).
- Medição de uso por inquilino e exportação do inquilino. Itens L7-09 e L7-25 pendentes. Dizer que
  o limite técnico existe na fundação. Dizer que a medição para cobrança não existe.
...
Gerado de laco/PAINEL.md (seção Fronteira) em 2026-09-08. Não editar entre os marcadores: rode `venv/bin/python docs/gerar_demo.py --preencher` depois de regenerar o painel.
