...
| `plat_tiles_requisicoes_total` | counter | `origem`, `resultado` | Requisições de ladrilho/COG que a **API** autoriza diretamente (ex.: verificação de token antes de servir um recorte). Incrementada pela rota de autorização de ladrilho da API (`app/tiles/rotas.py::verificar`, o `auth_request` que o nginx consulta antes de repassar ao Martin): `origem="tiles_verificar"` e `resultado` = `autorizado` ou o motivo da recusa (vocabulário fechado de `_motivo`: `token_invalido`, `escopo_insuficiente`, `leitor_indisponivel`), nunca o token nem o caminho do ladrilho. A autorização de COG por Range (item L1, `_cog/autorizar`) ainda está em outro ramo e acrescentará outra `origem` quando entrar. Martin tem sua PRÓPRIA família nativa (§3) e não passa por aqui. |
