...
async function iniciar() {
  ...
  /* link para o executor de páginas (L5-01-a, já no tronco): só faz sentido no tipo `app` e com item salvo */
  const linkExecutar = documento.tipo === 'app' && id
    ? h('a', { id: 'executar', class: 'pequeno', href: `/executar?item=${id}`, target: '_blank', rel: 'noopener' },
      t('construtor.executar'))
    : null;
  ...
  seletorIdioma.addEventListener('change', async () => {
    ...
    if (linkExecutar) linkExecutar.textContent = t('construtor.executar');
  ...
  principal.append(
    h('div', { class: 'linha-ferramentas' }, btSalvar, btPublicar, estado, linkExecutar,
      h('label', { class: 'campo-idioma' }, seletorIdioma)),
    alvo,
  );
