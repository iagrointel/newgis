...
async function iniciar() {
  ...
  principal.append(
    areaRecuperacao,
    areaConflito,
    h('div', { class: 'linha-ferramentas' }, btDesfazer, btRefazer, btSalvar, btDiferenca, estado),
    areaDiferenca,
    alvo,
  );
    ...
    if (versaoServidor != null) {
      ...
      if (rServ.status === 200) {
        ...
        if (docServidor) {
          ...
          painel.append(h('h4', {}, 'diferença entre a versão do servidor e a sua'), montarArvoreDiferenca(h, comparacao, PALETA_LAYOUT));
        }
      }
    }
  ...
  btDiferenca.addEventListener('click', async () => {
    ...
    bt.addEventListener('click', async () => {
      ...
      limparEl(resultado);
      resultado.append(montarArvoreDiferenca(h, comparacao, PALETA_LAYOUT));
    });
