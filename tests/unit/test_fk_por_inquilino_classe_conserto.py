...
SUA apontando para o `id` de uma linha do inquilino A, numa amostra dos formatos que a migração
tocou (alvo comum tipo `item`/`pasta`, auto-referência uuid tipo `pasta.pai_id`, auto-referência
inteira tipo `token_servico.renovado_por`, e cadeia de duas tabelas tipo
`compartilhamento_link_item`). A FK tem de recusar SEMPRE — com o id de A (existe, não é seu) e com
um id inventado (não existe) — e a MENSAGEM de recusa do Postgres tem de ser a mesma nos dois casos,
matando o oráculo de existência."""
