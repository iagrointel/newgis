"""Testes do módulo campo: duas frentes no mesmo arquivo (o próprio módulo `app/campo/rotas.py` junta as duas,
`router_pwa` e `router`).
...
Testes do item L2-07-campo (fila de trabalho, roteiro do dia e visita com foto). Camada de apoio própria
(`zt_campo_`, mesmo padrão de tests/api/test_agol.py::_criar_camada_hospedada) — uma tabela mínima com
`globalid` (a mesma chave que `GET /api/camadas/{id}/feicoes/{globalid}` usa) e alguns pontos.
