...
3. importador BDGD contra um recorte de DISTRIBUIDORA REAL (1 alimentador de uma cooperativa real,
...
na distribuidora inteira, 26.581 linhas, E na BDGD completa, 2.418.764 linhas — três amostras
...
número que pareça completo. Se o ativo real de `tests/dados/gerar_bdgd_extrato.py` não existir nesta máquina
...
def extrato_real():
    """Um alimentador (CTMT) de uma cooperativa real — não a distribuidora inteira: medido que
...
def test_importa_distribuidora_real_e_confere_contagem(rede_eletrica, extrato_real):
    ...
    real) — a distribuidora INTEIRA foi tentada e medida como lenta demais para a suíte (ver
