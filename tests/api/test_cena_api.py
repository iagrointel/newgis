...
def _corpo_cena(camada_id=UUID_A, **mudancas):
    corpo = {
        "camera": {"centro": [-46.59, -23.49], "zoom": 15.5, "inclinacao": 60, "rotacao": 20},
        "camadas": [{"id": ULID_A, "camada_id": camada_id, "desenho": "extrusao",
...
def camada_real(sessao_a):
    """Uma camada vetorial que o inquilino demo lê. A cena declara relação com a camada que cita
    (app/catalogo/relacoes.py), e o catálogo recusa relação com item que o ator não lê — por isso todo
    documento de cena VÁLIDO deste arquivo aponta para uma camada de verdade."""
    r = sessao_a.get("/api/mapa/camadas")
    assert r.status_code == 200, r.text
    servivel = [c for c in r.json()["camadas"] if c["servivel"]]
    if not servivel:
        pytest.skip("sem camada vetorial no inquilino demo: rode scripts/cena_demo.py criar")
    return servivel[0]["id"]
...
def test_cena_valida_e_gravada_e_relida(sessao_a, cena_criada, camada_real):
    r = cena_criada(_corpo_cena(camada_real))
...
def test_cena_que_cita_camada_que_o_ator_nao_le_e_recusada(cena_criada):
    """Relação com item que a sessão não lê é recusada pelo catálogo (mesma regra de todo documento que
    cita outro item): a cena não vira uma porta para descobrir id de outro inquilino."""
    r = cena_criada(_corpo_cena(UUID_A))
    assert r.status_code == 422, r.text


def test_camera_fora_da_faixa_e_422_do_esquema_do_tipo(cena_criada, camada_real):
    dados = _corpo_cena(camada_real)
...
def test_slide_que_cita_camada_ausente_e_422_da_cena(cena_criada, camada_real):
    dados = _corpo_cena(camada_real)
...
def test_extrusao_sem_altura_e_422_da_cena(cena_criada, camada_real):
    dados = _corpo_cena(camada_real)
...
def test_edicao_tambem_passa_pelas_duas_validacoes(sessao_a, cena_criada, camada_real):
    r = cena_criada(_corpo_cena(camada_real))
    assert r.status_code == 201, r.text
    iid = r.json()["id"]
    ruim = _corpo_cena(camada_real)
...
def test_a_cena_declara_a_camada_como_relacao_do_item(sessao_a, cena_criada, camada_real):
    """`cena` já estava no extrator de relações do catálogo (app/catalogo/relacoes.py): a camada citada
    no corpo vira relação, e é assim que o "usado por" da camada enxerga a cena. O id da camada está em
    `camada_id`, não em `id` (que na cena é o ULID citado pelos slides)."""
    r = cena_criada(_corpo_cena(camada_real))
    assert r.status_code == 201, r.text
    usado = sessao_a.get(f"/api/itens/{camada_real}/usado-por")
