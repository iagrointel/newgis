...
def test_raster_adversario_recusado_na_gravacao(sessao_a, itens_a):
    """Refutação do item L2-02-f contra a API real (sem Garage: a recusa acontece na validação do
    documento, antes de qualquer ladrilho): rescale invertido, banda inexistente, expressão com
    divisão por zero literal e colormap fora do vocabulário do TiTiler saem 422 com o campo apontado;
    o documento legítimo (PC_RASTER) passa no teste de round-trip acima."""
    # maplibre montado à mão: `_documento` compilaria o construtor inválido no próprio teste
    ml = {"version": 8, "layers": [{"id": "camada", "type": "raster"}]}
    for parametros_raster in (
        {"rescale": [3000, 0]},
        {"bandas": [99]},
        {"expression": "b4/0"},
        {"colormap_name": "rampa-inventada", "rescale": [0, 1]},
    ):
        pc = {"tipo": "raster", "geometria": "raster", "versao": 1, "parametros_raster": parametros_raster}
        r = criar_estilo(sessao_a, itens_a, pc, maplibre=ml)
        assert r.status_code == 422, (parametros_raster, r.text)
        # duas recusas legítimas: o esquema JSON barra na borda (dados_invalidos, ex.: banda 99 > 64)
        # e o compilador barra o que o esquema não sabe dizer (plat_construtor_invalido)
        assert r.json()["erro"] in ("plat_construtor_invalido", "dados_invalidos"), r.text

    # rampa com 1 classe (o 4o caso do adversário): a rampa nomeada é contínua por construção — o que
    # existe de discreto no documento é `rescale`, e a forma degenerada dele (min == max) é recusada
    pc = {"tipo": "raster", "geometria": "raster", "versao": 1,
          "parametros_raster": {"colormap_name": "viridis", "rescale": [5, 5]}}
    r = criar_estilo(sessao_a, itens_a, pc, maplibre=ml)
