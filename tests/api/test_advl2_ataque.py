...
from tests.api.test_dominios_subtipos import Camada, Inquilino  # fixtures de apoio
...
def test_2_referencia_a_camada_de_outro_inquilino_404_no_salvar(par):
    ...
    r = a.admin.post(
        ...
        json={
            "titulo": "zt cruzado",
            "dados": {"esquema_versao": 1, "corpo": {"camadas": [{"id": _ulid(), "ref": cam_b.item_id}]}},
        },
    )
    ...
    r2 = a.admin.post(
        "/api/mapas",
        json={
            "titulo": "zt inexistente",
            "dados": {"esquema_versao": 1, "corpo": {"camadas": [{"id": _ulid(), "ref": inexistente}]}},
        },
    )
...
def test_2_404_alheio_e_inexistente_nao_diferem_no_tempo(par):
    ...
    assert dif_ms < 3.0 and razao < 1.5, (
        f"alheio={m_alheio * 1000:.3f}ms inex={m_inex * 1000:.3f}ms dif={dif_ms:.3f}ms razao={razao:.2f}"
    )
...
@pytest.mark.xfail(
    strict=True,
    raises=psycopg2.errors.SyntaxError,
...
def test_3_injecao_dollar_tag_no_nome_do_dominio(par, con2):
    ...
    assert (
        a.admin.post(f"/api/camadas/{cam_a.item_id}/dominios", json={"campo": "uf", "dominio_id": d["id"]}).status_code
        == 201
    )
    ...
    with con2.cursor() as cur:
        cur.execute("UPDATE plat.dominio SET nome = %s WHERE id = %s::uuid", ("regiao $corpo_gerado$ x", d["id"]))
    con2.commit()
    # Se o gerador fosse robusto, um código válido entraria e um inválido seria recusado com nome nomeado:
    contexto(con2, a.id, usuario_id=a.admin_id, login="admin")
    with con2.cursor() as cur:
        cur.execute(
            f"INSERT INTO {cam_a.esquema}.{cam_a.tabela} (geom, uf) "
...
def test_3_dois_inquilinos_mesmo_nome_de_dominio_nao_colidem(par):
    ...
    da = _dominio(
        a.admin, nome=nome, tipo="codificado", tipo_campo="text", valores=[{"codigo": "SP", "descricao": "x"}]
    )
    db = _dominio(
        b.admin, nome=nome, tipo="codificado", tipo_campo="text", valores=[{"codigo": "RS", "descricao": "z"}]
    )
    assert (
        a.admin.post(f"/api/camadas/{cam_a.item_id}/dominios", json={"campo": "uf", "dominio_id": da["id"]}).status_code
        == 201
    )
    assert (
        b.admin.post(f"/api/camadas/{cam_b.item_id}/dominios", json={"campo": "uf", "dominio_id": db["id"]}).status_code
        == 201
    )
...
def test_3_valor_em_uso_conta_so_o_proprio_inquilino(par):
    ...
    r = a.admin.put(
        f"/api/dominios/{da['id']}",
        json={
            "nome": "zt dom " + secrets.token_hex(3),
            "tipo": "codificado",
            "tipo_campo": "text",
...
def leitor_conn(env, tmp_path_factory):
    ...
    r = subprocess.run(
        [
            ...
            "-n",
            "env",
            f"PLAT_SCHEMA={esquema}",
            f"CRED_DIR={cred}",
            f"CRED_DONO={os.environ.get('USER') or os.getlogin()}",
            "bash",
            str(ROOT / "db" / "leitor_instalar.sh"),
...
def camada_com_token(env, sessao_plat, con2):
    ...
    for cam in (cam_x, cam_y):
        ...
        with con2.cursor() as cur:
            cur.execute(
                f"INSERT INTO {cam.esquema}.{cam.tabela} (geom, nome) "
    ...
    with con2.cursor() as cur:
        cur.execute(
            ...
            "VALUES (%s,%s,%s,%s,%s,%s)",
            (inq.id, inq.admin_id, "zt-adv-" + secrets.token_hex(3), hash_, valor[:8], ["camada:ler:" + cam_x.item_id]),
        )
    con2.commit()
...
def test_1_token_escopo_de_camada_X_recusado_na_funcao_de_tile_de_Y(leitor_conn, camada_com_token):
    ...
    with leitor_conn.cursor() as cur, pytest.raises(psycopg2.Error) as e:
        cur.execute(
            f'SELECT "{c["cam_y"].esquema}"."t_{c["cam_y"].tabela[2:]}"(0,0,0,%s::json)',
...
@pytest.mark.xfail(
    strict=True,
    reason="ACHADO adversario (fronteira): a PROVA que a RLS do leitor exige e por "
...
def test_1_escopo_por_item_nao_e_forcado_no_select_direto(leitor_conn, camada_com_token):
    ...
    with leitor_conn.cursor() as cur:
        cur.execute(
            "SELECT plat.contexto_por_token(%s,NULL,NULL,%s::uuid,'camada:ler') t", (c["token_x"], c["cam_x"].item_id)
        )
