...
def _postgres(sql: str) -> str:
    ...
    return "\n".join(linha for linha in r.stdout.splitlines() if linha.strip() and not ruido.match(linha.strip()))
...
@pytest.mark.xfail(strict=True,
                   reason="L0-05-a: ordenacao global e prioridade 1..9 livre ao usuario; "
                          "sem justica entre inquilinos")
def test_fila_serve_o_inquilino_que_chegou_primeiro():
    ...
    saida = _postgres("""
      ...
      ROLLBACK;""")
    posicao = int([linha for linha in saida.splitlines() if linha.strip()][-1])
...
@pytest.mark.xfail(strict=True,
                   reason="L0-05-d: as chaves dos periodicos sao constantes e qualquer "
                          "inquilino pode ocupa-las")
...
@pytest.mark.xfail(strict=True,
                   reason="L0-05-a: filho.py grava traceback.format_exception() cru em "
                          "plat.job_log; nao ha saneamento")
def test_traceback_do_job_que_falhou_e_saneado(cliente, sessao_a):
    ...
    texto = "\n".join(linha["mensagem"] for linha in linhas)
...
@pytest.mark.xfail(strict=True,
                   reason="L0-05-b refutacao: _por_usuario e contador em memoria do processo "
                          "e a unidade sobe --workers 2")
...
@pytest.mark.xfail(strict=True,
                   reason="L0-05-e: item marcado entregue com o portao ainda no texto "
                          "'portao a fixar pelo arquiteto'")
