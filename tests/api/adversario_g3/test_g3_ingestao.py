...
# CONSERTADO em 06/09 (migração 20260906T1615a3f): plat.camada_schema_prefixo() devolve
# 'd_' + <schema da instalação> + '_', gravado como literal no corpo de camada_schema_garantir.
...
# CONSERTADO em 06/09: a contabilidade saiu do código de carga e virou o gatilho plat.item_uso_bytes,
# simétrico por construção (soma no INSERT do item, devolve no DELETE).
def test_uso_de_armazenamento_e_devolvido_quando_a_camada_e_apagada(ingestor_a, sessao_a, con_pg_adv):
    ...
    # expurgo físico (mesma chamada do periódico catalogo.lixeira_expurgar). `dias: 0` é obrigatório: sem
    # parâmetro o padrão é 30 dias e o job conclui com "0 itens a expurgar" — o roteiro original nunca chegou
    # a expurgar nada, e o que ele mediu foi o item ainda NA LIXEIRA, cujos bytes seguem ocupados de verdade.
    r = sessao_a.post("/api/jobs", json={"tipo": "catalogo.lixeira_expurgar", "parametros": {"dias": 0}})
