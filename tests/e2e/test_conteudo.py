"""e2e /conteudo (ADR 0004 seção 15; itens L0-03-catalogo e L0-03-f-tela-conteudo): as 5 abas (Meu conteúdo,
Favoritos, Meus grupos, Inquilino, Lixeira) e as 3 vistas (tabela, lista, grade) com captura, criar pasta e item
pela tela, editar título e tags em linha, compartilhar com o inquilino e por link (link anônimo 200 → revogar →
404), busca por campo (com erro de sintaxe no cliente), favoritar, mover para pasta, apagar → lixeira → restaurar
(mesmo uuid) → apagar agora; 0 erro de console; medidas primeira_pintura_conteudo_ms, pagina_conteudo_ms e
soma_modulos_kb. Salta enquanto o OpenAPI da URL interna não lista as rotas do catálogo.
Sem nginx na frente (uvicorn de trilha), o navegador passa pela frente de tests/e2e/frente_trilha.py (item
L0-03-e: cada trilha sobe a própria API numa porta e nunca toca o nginx da máquina)."""
...
from tests.e2e.apoio_catalogo import TelaCatalogo, gravar_medidas_catalogo
...
def test_conteudo_fluxo_completo(browser, base_url, credenciais_demo, admin_api, api_catalogo, playwright, medida, env):
    slug, admin_login, senha_admin = credenciais_demo
    s = sufixo()
    titulo = f"E2E mapa {s}"
    titulo_novo = f"E2E mapa {s} editado"
    nome_pasta = f"E2E pasta {s}"
    nome_grupo = f"E2E grupo conteudo {s}"
    frente = FrenteTrilha.se_preciso(base_url, env["PLAT_URL_PUBLICA"])
    item_id = pasta_id = grupo_id = None
    with frente or contextlib.nullcontext():
        ...
        try:
            ...
            tela.medidas["primeira_pintura_conteudo_ms"] = (
            ...
            tela.capturar("tabela")
            # meus grupos: cria um grupo (o gatilho do banco já inclui o criador como dono/membro),
            # a aba mostra o seletor e a lista do grupo (vazia, mas sem erro de console)
            r = admin_api.post("/api/grupos", data={"nome": nome_grupo, "visibilidade": "inquilino"})
            assert r.status == 201, r.text()
            grupo_id = r.json()["id"]
            page.click("#aba-grupos")
            page.wait_for_selector("#grupo-seletor:not([hidden]) select", timeout=15000)
            page.locator("#grupo-seletor select").select_option(grupo_id)
            page.wait_for_function("() => !document.querySelector('#lista').getAttribute('aria-busy') "
                                    "|| document.querySelector('#lista').getAttribute('aria-busy') === 'false'",
                                    timeout=15000)
            tela.capturar("grupos")
            page.click("#aba-meus")
            page.wait_for_selector("#aba-meus[aria-selected='true']", timeout=10000)
            # pasta pela tela
            ...
            _limpar_aviso(page, "#aviso")
            page.locator("#lixeira-tabela tbody tr", has_text=titulo_novo).locator(
...
def test_conteudo_sem_chave_crua(browser, base_url, credenciais_demo, api_catalogo, env):
