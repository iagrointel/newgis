...
100 MiB -> 7 partes de 16 MiB, mesmo tamanho do portão da API), acompanha a barra de progresso passar por
"parte N de 7" (a rede real é rápida demais para garantir capturar exatamente "parte 1"; o teste aceita
qualquer N observado, o que importa é o mecanismo — texto de progresso mudando e barra chegando a 100) até o
aviso final "arquivo enviado", 0 erro de console. Capturas em
...
def test_upload_com_barra_de_progresso_ate_arquivo_enviado(page, base_url, credenciais_demo, api_uploads, medida):
    ...
    caminho = _csv_temporario(100 * 1024 * 1024)  # 7 partes de 16 MiB (a última menor)
    try:
        ...
        page.wait_for_function(
            "() => /parte \\d+ de 7/.test(document.querySelector('#upload-rotulo-progresso')?.textContent || '')",
            timeout=15000,
        )
        tela.capturar("progresso_parte")
        page.wait_for_function(
            "() => document.querySelector('#aviso')?.textContent?.includes('arquivo enviado')", timeout=60000,
        )
        tela.medidas["envio_100mb_ms"] = round((time.perf_counter() - t0) * 1000, 1)
