...
class _Manipulador(http.server.BaseHTTPRequestHandler):
    ...
    def _rotear(self, caminho: str, consulta: dict):
        ...
        if caminho == "/sharing/rest/generateToken":
            return self._responder({"token": self.cfg.get("token_emitido", "tok-de-mentira"),
                                    "expires": 9_999_999_999_000, "ssl": True})
        ...
        if caminho == f"/sharing/rest/portals/{org}/groups":
            return self._responder({"total": len(a["grupos"]), "start": 1, "num": len(a["grupos"]),
                                    "nextStart": -1, "groups": a["grupos"]})
        if caminho == f"/sharing/rest/portals/{org}/users":
            return self._responder({"total": len(a["usuarios"]), "start": 1, "num": len(a["usuarios"]),
                                    "nextStart": -1, "users": a["usuarios"]})
        ...
        if sinteticos:
            if not self.server.sinteticos:
                self.server.sinteticos = [
                    {"id": f"{i:032x}", "owner": "ana.tec", "title": f"Item sintético {i}", "type": "CSV",
                     "typeKeywords": ["CSV"], "created": 1_700_000_000_000, "modified": 1_700_000_000_000,
                     "lastViewed": 1_700_000_000_000, "url": None, "size": 100 + i, "numViews": i}
    ...
    def _buscar(self, consulta: dict) -> dict:
        ...
        proxima = inicio + num
        return {
            "query": consulta.get("q", ""), "total": len(itens), "start": inicio, "num": len(fatia),
    ...
    def _item(self, caminho: str, consulta: dict):
        ...
        if partes[1] == "resources":
            recursos = [{"resource": "info.json", "created": 1_700_000_000_000, "size": 12}] \
                if item["type"] in ("Web Map", "Form") else []
            return self._responder({"total": len(recursos), "start": 1, "num": len(recursos),
                                    "nextStart": -1, "resources": recursos})
...
class PortalFalso:
    ...
    def __enter__(self) -> PortalFalso:
        ip = ip_publico()
        if ip is None:
            raise RuntimeError("máquina sem IP público: o servidor de mentira não pode escutar em loopback "
