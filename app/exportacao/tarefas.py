    ...
    try:
        with ctx.db() as cur:
            ...
            if item is None:
                ...
                raise FalhaDefinitiva("a camada de origem não existe mais")
        ...
        campos = [c for c in (p.get("campos") or campos_item) if c in campos_item]
        ...
        elif formato.nome == "shapefile":
            ctx.progresso(55, "compactando o shapefile")
            final = trabalho / nome_arquivo
            motor.zipar_diretorio(alvo, final)
        ...
        feicoes = motor.contar_feicoes(final, formato, ctx.subprocesso)
