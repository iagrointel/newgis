...
class CursorSchemaAmbiente(MixinReescritaSchema, psycopg2.extras.RealDictCursor):
    ...
    def _reescrever_bytes(cls, sql: bytes) -> bytes:
        """Alias de compatibilidade só em bytes -> bytes, mantido por `tests/unit/test_schema_ambiente_bytes.py`
        (item L4-01-modelo-rede, anterior à virada para `MixinReescritaSchema`): a lógica é a MESMA
        `_reescrever` herdada do mixin (que já trata bytes), sem reimplementar nada aqui."""
        return cls._reescrever(sql)
