...
def executar_count(cur, schema: str, tabela: str, prep: dict) -> ResultadoCount:
    _executar(
        cur, f'SELECT count(*) AS n FROM {_origem(schema, tabela, prep)} WHERE {prep["where_sql"]}',
...
def executar_ids(cur, schema: str, tabela: str, prep: dict) -> ResultadoIds:
    ...
    _executar(cur, sql, [*_origem_params(prep), *prep["where_params"], MAX_IDS_SEM_LIMITE + 1])
    linhas = cur.fetchall()
...
def executar_features(cur, schema: str, tabela: str, prep: dict, p: PedidoQuery, meta: list[dict],
    ...
    _executar(cur, sql, [*select_params, *_origem_params(prep), *params, limite, offset])
    linhas = cur.fetchall()
