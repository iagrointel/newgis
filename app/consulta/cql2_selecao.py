...
Módulo separado de `app.consulta.cql2` (item L2-04-g/h, OGC API Features + FES): a fusão de ~146
ramos (10/09) colidiu os dois num arquivo só (`cql2.py`) e ficou com a implementação errada para
cada chamador — 156 testes de unit caíram. São dois desenhos genuinamente diferentes (este valida
tipo por coluna com `ErroWhere`/`ConsultaSQL`, sem CQL2-text; o outro fala CQL2-text+JSON completo
com `ErroCql2`) e nenhum é superconjunto do outro, então a correção (item de conserto de fusão,
16/09) foi separar os arquivos em vez de forçar uma função só a atender duas assinaturas.
`app.mapa.selecao` (L2-01-h) importa DESTE módulo; `app.consulta.fes` e as rotas OGC API Features
importam de `app.consulta.cql2`.
