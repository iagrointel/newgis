"""Tipos de job do backup lógico — DUAS frentes no mesmo arquivo (ver o desenho completo em
`app/backup/__init__.py`). Compartilham `_banco_nome`/`_contar_tabelas` (implementação idêntica nos
dois desenhos) e o núcleo puro de `app/backup/nucleo.py`; cada frente tem a própria `_notificar_falha*`
porque avisam por canais diferentes (a frente global também tenta e-mail ao superadmin via
`correio.enviar`; a por-inquilino só grava o evento).
...
def _notificar_falha_global(ctx, motivo: str, detalhe: dict) -> None:
...
def backup_dump_logico(ctx, min_livre_gb: int = nucleo.MIN_LIVRE_GB,
    ...
    try:
        ...
        _notificar_falha_global(ctx, str(e), {"livre_bytes": e.livre_bytes, "minimo_bytes": e.minimo_bytes,
    ...
    with ctx.db() as cur:
        try:
            ...
            _notificar_falha_global(ctx, f"destino do backup indisponível: {e}", {})
    ...
    for grupo, objetos_grupo in manifestos.items():
        doc = nucleo.manifesto_inquilino(grupo, objetos_grupo, agora)
...
def backup_restore_drill(ctx, somente: list[str] | None = None,
    ...
    if ruins:
        ...
        _notificar_falha_global(ctx, f"ensaio de restauração acusou divergência — {motivo}",
