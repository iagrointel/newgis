...
def enfileirar(tenant_id: int, tipo: str, parametros: dict, usuario_id: int | None = None,
    ...
    `agendado_para` agenda a execução para o futuro (o coletor do worker só pegar a partir de então); o
...
def registrar_concluido(tenant_id: int, tipo: str, parametros: dict, resultado: dict) -> str:
    ...
    with banco.db(ctx) as cur:
        ...
        # (SECURITY DEFINER, migração 1932): nasce pendente por causa do gatilho `plat.job_transicao`,
        # e a conclusão reusa `plat.job_terminar`, com os mesmos disparos de notificação do worker
        cur.execute(
