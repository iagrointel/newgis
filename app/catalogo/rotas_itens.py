...
def editar_item(
    ...
    if "base_versao" in campos:
        ...
        if base_versao is not None and base_versao != r["versao_atual"]:
            # L5-13: o cliente leu `base_versao` e o servidor já está adiante — mescla por nó (documento de
            # grafo) ou recusa com o documento atual; para item sem grafo vale a regra estrita do versao_atual
...
def _mesclar_com_base(cur, r: dict, campos: dict, base_versao: int) -> dict:
    """Mesclagem de três vias por nó (item L5-13, `app/catalogo/mesclagem.py`): base = corpo da versão que o
    cliente leu (`plat.item_versao`), servidor = `dados` atual, cliente = `campos["dados"]`. Sem conflito,
    `campos["dados"]` passa a ser o resultado mesclado e a edição segue; com conflito, 409 `versao_conflito`
    com o documento atual inteiro e os ids dos nós em conflito — nunca uma escolha às escondidas (refutação
    do item)."""
    ...
    if linha is None:
        raise ErroAPI(
            409, "versao_conflito",
            f"a versão base {base_versao} não existe mais (compactada ou inválida); recarregue",
            {"versao_atual": r["versao_atual"], "base_versao": base_versao, "dados": r["dados"]},
        )
