...
ISO 19115-3 (`mdb:MD_Metadata`) NÃO é `outputSchema` aqui: é a cláusula 2 do item, e ficou de fora desta
passagem porque o XSD `mdb`/19115-3 não está em `docs/xsd/cache/` e baixá-lo exige rede (ver `docs/PARIDADE.md`
e o relato do turno) — só ISO 19139/GMD, que já tem XSD cacheado e gerador pronto."""
...
def get_capabilities(base_url: str, titulo: str) -> bytes:
    ...
    catálogo, escapado). Não há XSD `CSW-discovery.xsd` cacheado nesta máquina (`docs/xsd/cache/` só tem a
    árvore ISO 19139 e a de WFS/WMS/WMTS) — a prova de que o XML é bem formado fica para o teste (parse com
    lxml); validar contra o XSD oficial do CSW ficaria pendente do mesmo jeito que ISO 19115-3 (cláusula 2)."""
    base = _base(base_url)
...
def get_records_response(
    ...
    iso = output_schema in OUTPUT_SCHEMA_ISO
    total, ids, _proximo, _aprox = listar_ids(cur, auth, p)
    base_api = base_url.rstrip("/")
    nsmap = dict(NS_ISO if iso else NS_RECORDS)
    ...
    resultados.set("recordSchema", GMD if iso else "http://www.opengis.net/cat/csw/2.0.2#Record")
    resultados.set("elementSet", "full")
    resultados.set("numberOfRecordsMatched", str(total))
    if iso:
        brutas = _linhas_brutas(cur, ids)
        registros = [metadado.montar_md_metadata(brutas[i], tenant_nome, base_api) for i in ids if i in brutas]
    else:
...
def get_record_by_id_response(row: dict, auth, base_url: str, output_schema: str, tenant_nome: str) -> bytes:
    ...
    if iso:
        root = etree.Element(root_tag, nsmap=NS_ISO)
        md = metadado.montar_md_metadata(row, tenant_nome, base_api)
        try:
            metadado.validar(etree.tostring(md))  # mesma validação de GET .../metadado.xml, XSD cacheado
