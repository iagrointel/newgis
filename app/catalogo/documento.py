...
FAMILIAS_GRAFO = {"app", "painel", "site", "narrativa"}
...
def erros_de_desenho(dados) -> list[dict]:
    ...
    for i, f in enumerate(features):
        ...
        if tipo_desenho not in TIPOS_DESENHO:
            saida.append(
                _erro(
                    f"{campo}.properties.tipo_desenho",
                    f"tipo de desenho desconhecido: {tipo_desenho!r}",
                    "tipo_desconhecido",
                )
            )
            tipo_desenho = None
        ...
        if not isinstance(geom, dict) or "type" not in geom or "coordinates" not in geom:
            ...
            if not _num_dentro_do_mundo(geom.get("coordinates")):
                saida.append(
                    _erro(f"{campo}.geometry.coordinates", "coordenada fora do mundo (lon/lat)", "fora_do_mundo")
                )
        ...
        if isinstance(rotulo, str) and len(rotulo) > DESENHO_TEXTO_MAX:
            saida.append(
                _erro(
                    f"{campo}.properties.rotulo",
                    f"{len(rotulo)} caracteres; o máximo é {DESENHO_TEXTO_MAX}",
                    "teto",
                )
            )
