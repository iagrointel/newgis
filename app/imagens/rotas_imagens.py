...
# --------------------------------------------------- conexão S3 do inquilino (Porta 2 do item L1-02-e)
@router.get("/api/imagens/conexao-s3", openapi_extra={"x-auth": "S", "x-privilegio": "conteudo.publicar_camada"})
def conexao_s3(request: Request, auth: Auth = autenticado("conteudo.publicar_camada", so_sessao=True)):
    ...
    o que faltava era uma porta que a entregasse ao dono do dado. Passo a passo em `docs/PRO_CONEXAO.md`.
    ...
    with db.db(auth.contexto()) as cur:
        bucket = objetos.garantir_bucket(cur, auth.tenant_id, auth.tenant_slug)
    if not settings.PLAT_GARAGE_URL:
        raise ErroAPI(503, "armazenamento_indisponivel",
                      "esta instalação não tem endpoint S3 configurado (PLAT_GARAGE_URL)")
    endpoint = settings.PLAT_GARAGE_URL
    with db.db(auth.contexto()) as cur:
        registrar_evento(cur, request, "imagens/conexao-s3", "inquilino", auth.tenant_id,
                         {"balde": bucket["bucket_alias"], "modo": "somente_leitura"})
    corpo = {
        "endpoint": endpoint,
        "endpoint_sem_esquema": endpoint.split("://", 1)[-1],
        "https": endpoint.startswith("https://"),
        "regiao": settings.PLAT_GARAGE_REGIAO,
        "balde": bucket["bucket_alias"],
        "modo": "somente_leitura",
        "access_key_id": bucket["chave_ro_id"],
        "secret_access_key": bucket["chave_ro_segredo"],
        "estilo_endereco": "path",  # o Garage fala S3 com o balde no CAMINHO, nunca no subdomínio
        "exemplo_gdal": f"/vsis3/{bucket['bucket_alias']}/<chave-do-objeto>",
        "documento": "docs/PRO_CONEXAO.md",
        "aviso": ("credencial somente-leitura do balde deste inquilino; ela abre TODO objeto do balde. "
                  "Para dar acesso a UM item, use token de serviço com escopo por lista (L1-02-b)."),
    }
