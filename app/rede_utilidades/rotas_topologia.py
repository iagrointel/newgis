...
# Bloco restaurado de da2330fd: a fusão de 17/09 ficou sem estas 2 rotas (os ajudantes de feicoes.py foram
# clobados junto); sem elas, GET .../feicoes/pontos.geojson respondia 404.
...
def _tracar_sincrono(rid: str, corpo: TracadoEntrada, auth: Auth, request: Request) -> dict:
    """Um motor só. A escolha do traçado por `tipo`/`config_id` mora em `despacho.executar`, que é o mesmo
    que exportar, salvar como camada e repetir usam — antes esta rota tinha uma CÓPIA dessa escolha, e a
    cópia ficou para trás do original (o tipo `isolamento` nunca chegou aqui, item L4-02-c; e a execução
    nunca entrava no histórico, item L4-02-f). As agregações e o `execucao_id` saem daqui pelo mesmo motivo:
    o portão do L4-02-f pede os dois na resposta do traçado, não só na exportação."""
    with db.db(auth.contexto()) as cur:
        _rede_existe(cur, rid)
        ...
        if auth.leitura_inquilino is None:
            # superadmin lendo outro inquilino não escreve no histórico da casa que ele visita
