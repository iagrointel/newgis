...
def importar_de_servico(corpo: ImportarEntrada, request: Request,
    ...
    with db.db(auth.contexto()) as cur:
        ...
        saida = servico.importar_dominios(cur, auth.usuario_id, corpo.fields, corpo.types, item, corpo.prefixo)
        registrar_evento(cur, request, "dominios/importar", "item", corpo.item_id,
                         {"criados": len(saida["criados"]), "reaproveitados": len(saida["reaproveitados"]),
                          "ligados": len(saida["ligados"]), "ignorados": len(saida["ignorados"])})
    return saida
